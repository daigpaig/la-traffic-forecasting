...
% Final mode: shows author info. Use the default (no option) for anonymized
% submission, or [preprint] for a named arXiv-style version.
...
\author{%
  ...
  1927 Orrington Ave, Evanston, IL \\
  \texttt{daigomoriwake2027@u.northwestern.edu}
}
...
\quad topology: physical $\to$ correlation           & ---                & $-0.115$  & $\approx 6\sigma$    & \textbf{Yes} \\
\quad operator: GCN $\to$ GAT (Iter 2$\to$3)         & ---                & $-0.028$  & $\approx 1.5\sigma$  & confounded \\
\quad operator: GAT $\to$ ChebConv K=2               & $11.929$           & $+0.009$  & $\approx 0\sigma$    & null \\
\quad training length: 5 $\to$ 10 epochs             & ---                & $-0.031$  & $t$-$p \approx 0.07$ & borderline \\
